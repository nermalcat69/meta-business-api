---
title: "Marketing Message API for Messenger Changelog"
source_url: https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/additional-resources
---
...
[`POST /<PAGE_ID>/messaging_customer_base/unsubscribe`](https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/grow-marketing-messages-audience#unsubscribe-api)
...
[`POST /act_<AD_ACCOUNT_ID>/messages`](https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/send-messages#step-3-send-a-marketing-message)
...
[`GET /<MESSAGE_CAMPAIGN_ID>`](https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/send-messages#get-a-campaign)
...
[`GET /act_<AD_ACCOUNT_ID>/message_delivery_estimate`](https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/send-messages#delivery-estimation)
...
[`POST /act_<AD_ACCOUNT_ID>/message_campaigns`](https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/send-messages)
...
[`GET /act_<AD_ACCOUNT_ID>/campaigns`](https://developers.facebook.com/documentation/business-messaging/messenger-platform/marketing-messages-on-messenger/send-messages)
...
#### Custom Audiences API REMOVE\_USER Now Preserves Opt-In Status

* When you send a DELETE/REMOVE\_USER request for a Custom Audiences API subscriber, the user is now removed from the custom audience while their overall marketing message opt-in remains intact.
* Previously, REMOVE\_USER triggered a full opt-out from all marketing messages.
...
* Fixed a bug where the AD\_ACCOUNT webhook payload was emitting the wrong ad account ID.
...
* The Send API now returns error code 1893047 (RECIPIENT\_NOT\_AVAILABLE) instead of a generic internal error when the recipient is not available for receiving the message.
...
#### Insights time\_range and date\_preset Support
...
* New specific error codes have been introduced to provide clearer feedback when message sending fails:
  * 2300018 (ATTACHMENT\_SAVE\_FAILURE): Returned when an attachment fails to save during message processing.
  * 2300027 (ATTACHMENT\_REUSE\_NOT\_ALLOWED): Returned when attempting to reuse an attachment that is not permitted for reuse.
  * 2300052 (RECIPIENT\_UNAVAILABLE): Returned when a recipient is no longer reachable or available to receive marketing messages.
...
* When a message send is rejected due to pacing limits, the error response now includes a remaining\_seconds field in error\_data.
...
* This replaces the generic TOO\_MANY\_CALLS error, making it easier to distinguish token query rate limits from other API rate limits.
