---
title: "Metrics API"
source_url: https://developers.facebook.com/documentation/business-messaging/whatsapp/flows/guides/versioning
---
...
Flow metrics are still being developed and may change as we improve our methodologies. Use these metrics for directional guidance, but use caution when using them for historical comparisons or strategic planning.

## Available flow metrics

All metrics are not real time. In some cases it may take a couple of hours for metric events to be ingested and become available for querying.
...
| User-Access-Token | This can be retrieved by copying the *Temporary access token* from your app which expires in 24 hours.  Alternatively, you can generate a [System User Access Token](https://developers.facebook.com/documentation/business-messaging/whatsapp/access-tokens#system-user-access-tokens). |
...
This endpoint allows you to retrieve a particular metric data points for a specified time period and granularity.
...
| --- | --- | --- |
| `name`*string* | Metric name. See [available metrics](https://developers.facebook.com/documentation/business-messaging/whatsapp/flows/guides/versioning#available_metrics). | ✓ |
| `granularity`*string* | Time granularity, value being one of:   * `DAY` - values are segmented by days * `HOUR` - values are segmented by hours * `LIFETIME` - values for the entire period | ✓ |
| `since`*string: YYYY-MM-DD* | Start of the time period. If not specified, the oldest allowed date will be used.  Oldest allowed date depends on the specified time granularity:   * `DAY` - 90 days from the current date * `HOUR` - 30 days from the current date |  |
...
| `metric.data_points.timestamp`*string: datetime in ISO 8601 format* | Timestamp of the beginning of the data point interval. |
| `metric.data_points.data`*array of key-value objects* | Metric specific data object. See [available metrics](https://developers.facebook.com/documentation/business-messaging/whatsapp/flows/guides/versioning#available_metrics) for details. |
