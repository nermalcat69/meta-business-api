...
For each registered business phone number, Cloud API supports up to 80 messages per second (mps) by default, and up to [1,000 mps](https://developers.facebook.com/documentation/business-messaging/whatsapp/business-phone-numbers/phone-numbers#higher-throughput) by automatic upgrade.
...
If you meet our [eligibility requirements](https://developers.facebook.com/documentation/business-messaging/whatsapp/business-phone-numbers/phone-numbers#eligibility), your business phone number is automatically upgraded to 1,000 mps at no cost to you. Higher throughput does not incur additional charges or affect pricing.
...
Once your number is upgraded to higher throughput, a [phone\_number\_quality\_update](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/reference/phone_number_quality_update) webhook will be triggered with `event` set to `THROUGHPUT_UPGRADE` and `max_daily_conversations_per_business` set to `TIER_UNLIMITED`.
