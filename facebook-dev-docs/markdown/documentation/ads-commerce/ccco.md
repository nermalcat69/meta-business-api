...
source_url: https://developers.facebook.com/documentation/ads-commerce/ccco
---
...
Bid multipliers are going away soon. You can now replace some of your existing bid multipliers with **value rules** in the Marketing API to set different bid adjustments for different audience segments. [About value rules⁠](https://www.facebook.com/business/help/535014515741813)

Bid multipliers allow you to employ an audience-aware bidding strategy within a single ad set, by assigning different weights to different audience segments within your [**bid multiplier parameter set**](https://developers.facebook.com/documentation/ads-commerce/ccco#bid_multiplier_parameter_set). To bid differently for different audience segments **without bid multipliers**, you would typically create multiple ad sets, with each ad set targeting one specific audience segment. Using bid multipliers, you can create a single ad set with broad targeting, then attach a **bid multiplier parameter set** to specify bid adjustments for each audience segment. Bid multipliers can greatly reduce the number of ad sets and targeting segments needed to execute your bidding strategy.

Advertisers running housing, employment, and credit ads, who are based in the United States or are running ads targeted to the United States have different sets of restrictions and a subset of audience categories are available for adjusting bids. Housing, employment, and credit campaigns are restricted from adjusting bids for audience categories of [`age`](https://developers.facebook.com/documentation/ads-commerce/ccco#age), [`gender`](https://developers.facebook.com/documentation/ads-commerce/ccco#gender), [`locale`](https://developers.facebook.com/documentation/ads-commerce/ccco#locale), [`home_location`](https://developers.facebook.com/documentation/ads-commerce/ccco#home_location), [`user_bucket`](https://developers.facebook.com/documentation/ads-commerce/ccco#user_bucket), and lookalike [`custom_audience`](https://developers.facebook.com/documentation/ads-commerce/ccco#custom_audience) (a custom audience derived from a lookalike audience). See [**Special Ad Category**](https://developers.facebook.com/documentation/ads-commerce/marketing-api/audiences/special-ad-category).

Beginning January 30, 2023, ads that use a bid multiplier with third-party data categories (`booking_window`, `custom_audience` (including lookalike custom audiences), `length_of_stay`, `travel_start_date`, `travel_start_day_of_week`, `user_recency`, `user_bucket`) will no longer be delivered to users who have opted out.
...
This section assumes some familiarity with bid multipliers concepts and usage. See the [**How to Use Bid Multipliers**](https://developers.facebook.com/documentation/ads-commerce/ccco#howto) section for more details about using bid multipliers.

### Create value rule sets

This section demonstrates how to create value rules from scratch. If you are interested in directly migrating your existing ad sets using bid multipliers to value rules, you can skip to the [translation API](https://developers.facebook.com/documentation/ads-commerce/ccco#value_rules_translate) section for instructions on how to use our automated migration APIs.

Unlike bid multipliers, which require each ad set to be configured with its own [**bid multiplier parameter set**](https://developers.facebook.com/documentation/ads-commerce/ccco#bid_multiplier_parameter_set), **value rules** allow you to create **value rule sets** in your ad account. Once created, each **value rule set** can be attached to any number of ad sets.
...
Once an ad set has been migrated to value rules, the deletion of its **bid multipliers parameter set** cannot be reversed. Thus, it may be prudent to first [retrieve the bid multiplier parameter set](https://developers.facebook.com/documentation/ads-commerce/ccco#read) on the ad set prior to migrating it to value rules. Saving this retrieved **bid multiplier parameter set** on your systems will allow you to migrate the ad set back to bid multipliers if needed.
...
Next, use the [bid multipliers create/update API](https://developers.facebook.com/documentation/ads-commerce/ccco#update) to re-apply the **bid multiplier parameter set** to the ad set.
...
| --- | --- |
| `AGE` | [`age`](https://developers.facebook.com/documentation/ads-commerce/ccco#age) |
| `DEVICE_PLATFORM` | [`device_platform`](https://developers.facebook.com/documentation/ads-commerce/ccco#device_platform) |
| `GENDER` | [`gender`](https://developers.facebook.com/documentation/ads-commerce/ccco#gender) |
| `LOCATION` | [`home_location`](https://developers.facebook.com/documentation/ads-commerce/ccco#home_location) |
| `OMNI_CHANNEL` | Not available in bid multipliers |
| `PLACEMENT` | [`position_type`](https://developers.facebook.com/documentation/ads-commerce/ccco#position_type), [`publisher_platform`](https://developers.facebook.com/documentation/ads-commerce/ccco#publisher_platform) |
| `OS_TYPE` | [`user_os`](https://developers.facebook.com/documentation/ads-commerce/ccco#user_os) |

Note that both `publisher_platform` and `position_type` audience categories from bid multipliers map to the `PLACEMENT` criteria type in value rules. Refer to the [publisher\_platform](https://developers.facebook.com/documentation/ads-commerce/ccco#publisher_platform) and [position\_type](https://developers.facebook.com/documentation/ads-commerce/ccco#position_type) sections for details on how to translate these audience categories to value rules.
...
Due to [value rules limitations](https://developers.facebook.com/documentation/ads-commerce/ccco#value_rules_limits), not all bid multiplier parameter sets can be automatically translated to value rule sets. If the translation API cannot provide an automatic translation for your use case, you may still be able to create a value rule set that covers a portion of the functionality of your bid multiplier parameter set. Refer to the [manual translation](https://developers.facebook.com/documentation/ads-commerce/ccco#value_rules_translate_manual) section below for instructions on performing the translation process yourself.
...
Refer to the [Audience Category Reference](https://developers.facebook.com/documentation/ads-commerce/ccco#categories) section for a full list of audience categories.
...
You can find `city id` and `region id` in the [Search API](https://developers.facebook.com/documentation/ads-commerce/marketing-api/audiences/reference/targeting-search). You can query from the [Graph API Explorer](https://developers.facebook.com/tools/explorer/) or from your terminal.
...
You can find locale IDs with [Targeting Search, Locale](https://developers.facebook.com/documentation/ads-commerce/marketing-api/audiences/reference/targeting-search#locale) with `type=adlocale`.
...
See other possible values at [Targeting Search API](https://developers.facebook.com/documentation/ads-commerce/marketing-api/audiences/reference/targeting-search#country_group) with `type=adTargetingCategory` and `class=user_device`.
...
See other possible values at [Targeting Search API](https://developers.facebook.com/documentation/ads-commerce/marketing-api/audiences/reference/targeting-search#country_group) with `type=adTargetingCategory` and `class=user_os`.
