...
source_url: https://developers.facebook.com/documentation/ads-commerce/marketing-api/reference/business/third_party_partner_panel_requests
---
...
| --- | --- |
| `cadence` *string* | Optional filter on the request cadence.   ---   Cadence has the form "+[quantity] [day|week|month]"  Valid input examples:   * cadence=+1 day * cadence='+1 day' |
...
| `status` *array<enum {CANCELLED, CREATED, FINISHED, ONGOING}>* | Optional filter on the request status.   ---   From the set { CANCELLED, CREATED, FINISHED, ONGOING }  Can enter a single study type or an array of statuses. The array filter is a set contains, e.g. if a business has a request with 'CREATED', a request with 'FINISHED', and has filter status=['CREATED', 'FINISHED'], then both requests will be returned.   ---   Valid input examples:   * status=CREATED * status=['CREATED', 'FINISHED']   Invalid input examples:   * country='CREATED' * country=[CREATED, FINISHED] |
